...
func SetupRouter(
	...
	api := r.Group("/api")
	{
		...
		api.GET("/system/local-ips", systemHandler.GetLocalIPs)
		api.GET("/proxy/mode", systemHandler.GetProxyMode)
		api.PUT("/proxy/mode", systemHandler.SetProxyMode)
		api.POST("/system-proxy/enable", systemHandler.EnableSystemProxy)
		api.POST("/system-proxy/disable", systemHandler.DisableSystemProxy)
		api.GET("/system-proxy/status", systemHandler.GetSystemProxyStatus)
